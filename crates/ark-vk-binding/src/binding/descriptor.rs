...
use crate::{
    binding::{
        ark::gpu::{
            core::VulkanError,
            descriptor::{
                DescriptorBinding, DescriptorBindingFlags, DescriptorPoolCreateFlags,
                DescriptorType, DescriptorWrite, Host, HostDescriptorPool, HostDescriptorSet,
...
impl HostDescriptorSetLayout for VkContextView<'_> {
    fn create(
    ...
    ) -> Result<Resource<super::ark::gpu::descriptor::DescriptorSetLayout>, VulkanError> {
        ...
        let layout =
            unsafe { self.vk_device().create_descriptor_set_layout(&layout_info, None) }
                .map_err(vk_err)?;
        ...
        let handle = self.table.push(gpu_layout).map_err(|_| VulkanError::OutOfHostMemory)?;
        Ok(Resource::new_own(handle.rep()))
    }

    fn drop(&mut self, rep: Resource<super::ark::gpu::descriptor::DescriptorSetLayout>) -> wasmtime::anyhow::Result<()> {
        let key = Resource::<GpuDescriptorSetLayout>::new_own(rep.rep());
        let layout = self.table.delete(key)?;
        unsafe { self.vk_device().destroy_descriptor_set_layout(layout.layout, None); }
        Ok(())
    }
}
...
impl HostDescriptorPool for VkContextView<'_> {
    fn create(
    ...
    ) -> Result<Resource<super::ark::gpu::descriptor::DescriptorPool>, VulkanError> {
        ...
        let gpu_pool = GpuDescriptorPool { pool };
        let handle = self.table.push(gpu_pool).map_err(|_| VulkanError::OutOfHostMemory)?;
        Ok(Resource::new_own(handle.rep()))
    }

    fn drop(&mut self, rep: Resource<super::ark::gpu::descriptor::DescriptorPool>) -> wasmtime::anyhow::Result<()> {
        let key = Resource::<GpuDescriptorPool>::new_own(rep.rep());
        let pool = self.table.delete(key)?;
        unsafe { self.vk_device().destroy_descriptor_pool(pool.pool, None); }
        Ok(())
    }
}
...
impl HostDescriptorSet for VkContextView<'_> {
    fn allocate(
        &mut self,
        pool: Resource<super::ark::gpu::descriptor::DescriptorPool>,
        layout: Resource<super::ark::gpu::descriptor::DescriptorSetLayout>,
        variable_descriptor_counts: Vec<u32>,
    ) -> Result<Resource<super::ark::gpu::descriptor::DescriptorSet>, VulkanError> {
        let pool_key = Resource::<GpuDescriptorPool>::new_borrow(pool.rep());
        let gpu_pool = self.table.get(&pool_key).map_err(|_| VulkanError::Unknown)?;
        ...
        let gpu_set = GpuDescriptorSet { set: sets[0] };
        let handle = self.table.push(gpu_set).map_err(|_| VulkanError::OutOfHostMemory)?;
        Ok(Resource::new_own(handle.rep()))
    }

    fn write(
        &mut self,
        self_: Resource<super::ark::gpu::descriptor::DescriptorSet>,
        writes: Vec<DescriptorWrite>,
    ) -> Result<(), VulkanError> {
        let set_key = Resource::<GpuDescriptorSet>::new_borrow(self_.rep());
        ...
        for write in &writes {
            ...
            let mut wd = vk::WriteDescriptorSet::builder()
                .dst_set(gpu_set.set)
            ...
            if let Some(ref buf_info) = write.buffer_info {
                let buf_key = Resource::<super::buffer::GpuBuffer>::new_borrow(buf_info.buffer.rep());
                let buf = self.table.get(&buf_key).map_err(|_| VulkanError::InvalidBuffer)?;
                buffer_infos.push(
            ...
            if let Some(ref img_info) = write.image_info {
                ...
                wd = wd.image_info(std::slice::from_ref(&image_infos[image_infos.len() - 1]));
            }

            write_descs.push(wd.build());
        }
    ...
    fn drop(&mut self, rep: Resource<super::ark::gpu::descriptor::DescriptorSet>) -> wasmtime::anyhow::Result<()> {
